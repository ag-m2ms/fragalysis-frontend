...
function DeleteModal({ show, handleDeleteClose, handleDelete, id, projectName }) {
...
function CreateModal({ show, handleCreateClose, handleCreate, projectName, id }) {
  ...
  return (
    <Modal show={show} onHide={handleCreateClose} animation={false}>
      ...
      <Modal.Body>Would you like to create auto-generate protocols for project: {projectName}?</Modal.Body>
      <Modal.Footer>
      ...
      <Navbar.Collapse id="basic-navbar-nav">
        <Nav className="mr-auto">
          ...
          <NavDropdown title="Delete project" id="collasible-nav-dropdown">
            {Projects.map(project => (
              <React.Fragment>
                <NavDropdown.Item key={project.id + 1} onClick={() => handleDeleteShow(project.name, project.id)}>
                  {project.name}
                </NavDropdown.Item>
              </React.Fragment>
            ))}
          ...
          <NavDropdown title="Create OT Protocol" id="collasible-nav-dropdown">
            {Projects.map(project => (
              <React.Fragment>
                <NavDropdown.Item key={project.id + 2} onClick={() => handleCreateShow(project.name)}>
                  {project.name}
                </NavDropdown.Item>
              </React.Fragment>
            ))}
